...
def due_summary(request):
    ...
    source_list = list(set(map(get_source,dues)))
    
    def get_income_source_amount(source):
        amount = 0
        source = IncomeSource.objects.get(source=source)
        filtered_by_source = dues.filter(source=source.id)
