...
def queryset_filter(filter_by):
    ...
    if filter_by.lower() == 'today':
        expenses = Expense.objects.filter(date = today_date_time.date())

    elif filter_by.lower() == 'week':
        week_date_time = today_date_time - timedelta(days=7)
        expenses = Expense.objects.filter(date__gte=week_date_time.date())

    elif filter_by.lower() == 'month':
        expenses = Expense.objects.filter(date__year=today_date_time.year,date__month=today_date_time.month)

    else:
        expenses = Expense.objects.filter(date__year=today_date_time.year)
