...
def queryset_filter(user,filter_by):
    ...
    if filter_by.lower() == 'today':
        dues = Due.objects.filter(user=user,date = today_date_time.date())

    elif filter_by.lower() == 'week':
        week_date_time = today_date_time - timedelta(days=7)
        dues = Due.objects.filter(user=user,date__gte=week_date_time.date())
    
    elif filter_by.lower() == 'month':
        dues = Due.objects.filter(user=user,date__year=today_date_time.year,date__month=today_date_time.month)
    
    else:
        dues = Due.objects.filter(user=user,date__year=today_date_time.year)
