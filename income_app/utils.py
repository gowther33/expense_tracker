...
def queryset_filter(filter_by):
    ...
    if filter_by.lower() == 'today':
        incomes = Income.objects.filter(date = today_date_time.date())

    elif filter_by.lower() == 'week':
        week_date_time = today_date_time - timedelta(days=7)
        incomes = Income.objects.filter(date__gte=week_date_time.date())
    
    elif filter_by.lower() == 'month':
        incomes = Income.objects.filter(date__year=today_date_time.year,date__month=today_date_time.month)
    
    else:
        incomes = Income.objects.filter(date__year=today_date_time.year)
